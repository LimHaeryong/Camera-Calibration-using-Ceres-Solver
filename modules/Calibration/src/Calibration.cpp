...
void Calibration::calibrate()
{
  ...
  for (const auto &image_path : image_paths_)
  {
    ...
    bool ret = cv::findChessboardCorners(
      image, pattern_size_, corners, FIND_CHESSBOARD_FLAGS);
    if (ret == false)
    {
      continue;
    }
    cv::cornerSubPix(
      image, corners, cv::Size(7, 7), cv::Size(-1, -1), CORNERSUBPIX_CRITERIA);
  ...
  std::vector<std::vector<cv::Point3f>> object_points(
    image_points.size(), object_point);
...
void Calibration::save_yaml()
{
  ...
  std::vector<double> camera_matrix(
    camera_matrix_.begin<double>(), camera_matrix_.end<double>());
  config["camera_matrix"] = camera_matrix;

  std::vector<double> dist_coeffs(
    dist_coeffs_.begin<double>(), dist_coeffs_.end<double>());
  ...
  cv::Mat &dist_coeffs)
{
  ...
  double reprojection_error = refine_all(
    camera_matrix, dist_coeffs, extrinsics, object_points, image_points);
...
void Calibration::get_intrinsics(
...
cv::Mat Calibration::get_extrinsic(
  const cv::Mat &camera_matrix,
...
cv::Vec3d Calibration::get_normalized_coord(
  ...
  for (std::size_t i = 0; i < M; ++i)
  {
    for (std::size_t j = 0; j < N; ++j)
    {
      ceres::CostFunction *cost_function =
        reprojection_error::create(image_points[i][j], object_points[i][j]);
      problem.AddResidualBlock(
        cost_function, NULL, intrinsic_vec.data(), extrinsics_vec[i].data());
    }
  }
